...
1. 修改`Responder.conf`里面的Challenge为`1122334455667788`(使用[SpiderLabs版本](https://github.com/SpiderLabs/Responder)的 话默认是`1122334455667788`，但该版本已经停止更新了，建议使用[lgandx版本](https://github.com/lgandx)，这一版本默认为`Random`，需要修改)
   ...
   ![image-20191201161721971](img/NTLM_RELAY_2_ANQUANKE/image-20191201161721971.png)

2. 将type2里面的NTLMSSP_NEGOTIATE_EXTENDED_SESSIONSECURITY位置0。

   如果知道发来的请求一定是SMB 协议的话，Responder里面加上`--lm`参数即可，

   ![image-20191201162547667](img/NTLM_RELAY_2_ANQUANKE/image-20191201162547667.png)
   ...
   ![，还成](img/NTLM_RELAY_2_ANQUANKE/image-20191201162653494.png)

   ![image-20191201162838255](img/NTLM_RELAY_2_ANQUANKE/image-20191201162838255.png)

3. 然后获取到Net-NTLM v1。再使用[ntlmv1-multi](https://github.com/evilmog/ntlmv1-multi)里面的ntlmv1.py转换.
![image-20191201171051773](img/NTLM_RELAY_2_ANQUANKE/image-20191201171051773.png)
 获取到Net-NTLM v1是`win10::WIN10-1:F1586DA184365E9431C22EF206F5A2C918659E1B1FD7F64D:F1586DA184365E9431C22EF206F5A2C918659E1B1FD7F64D:1122334455667788`
![image-20191201171244683](img/NTLM_RELAY_2_ANQUANKE/image-20191201171244683.png)
转化完的格式就是`NTHASH:F1586DA184365E9431C22EF206F5A2C918659E1B1FD7F64D`

4. 再将转化完的格式用[crack.sh](https://crack.sh/get-cracking/)破解即可。

![image-20191201173245315](img/NTLM_RELAY_2_ANQUANKE/image-20191201173245315.png)

![image-20191201174336446](img/NTLM_RELAY_2_ANQUANKE/image-20191201174336446.png)
...
4. 将这三个密文值连接起来得到response。
   ...
   在Responder如果想获取到这种加密方式的话，要加上参数`--lm`(仅限于smb 协议)![image-20191126122252915](img/NTLM_RELAY_2_ANQUANKE/image-20191126122252915.png)
   ...
   具体操作是![image-20191126122353928](img/NTLM_RELAY_2_ANQUANKE/image-20191126122353928.png)
...
- （2）加密方式2
  ...
  在Responder默认获取到的就是这种加密方式(没有加`--lm`)![image-20191126131040382](img/NTLM_RELAY_2_ANQUANKE/image-20191126131040382.png)

  使用[ntlmv1-multi](https://github.com/evilmog/ntlmv1-multi)里面的ntlmv1-ssp.py转换.![image-20191126131242128](img/NTLM_RELAY_2_ANQUANKE/image-20191126131242128.png)到[crack.sh](https://crack.sh/get-cracking/)破解。这种方式要钱的，而且还不一定能解的出来。![image-20191126131312228](img/NTLM_RELAY_2_ANQUANKE/image-20191126131312228.png)
...
![image-20191126133346842](img/NTLM_RELAY_2_ANQUANKE/image-20191126133346842.png)
...
那怎么将NTLMSSP_NEGOTIATE_EXTENDED_SESSIONSECURITY位置为0，我们一般都是使用现成的工具Resonder来获取Net-NTLM Hash。之前说过加上`--lm`参数就可以将NTLMSSP_NEGOTIATE_EXTENDED_SESSIONSECURITY位置为0。

这个时候还有一个小问题没有解决，那就是Resonder加上`--lm`，为什么只针对smb 协议有效。其他协议无效。

我去读了下Responder的代码。

![image-20191126134030335](img/NTLM_RELAY_2_ANQUANKE/image-20191126134030335.png)

加上`--lm`参数之后，调用的模块就是SMB1LM

![image-20191126134218565](img/NTLM_RELAY_2_ANQUANKE/image-20191126134218565.png)

发现她用的是老板本的smb实现。而这个版本的实现是在smb 协商版本的时候就将challenge返回，并且将NTLMSSP_NEGOTIATE_EXTENDED_SESSIONSECURITY置为0.

![image-20191126134410933](img/NTLM_RELAY_2_ANQUANKE/image-20191126134410933.png)
...
![image-20191126135028008](img/NTLM_RELAY_2_ANQUANKE/image-20191126135028008.png)

默认是0xe2898215(跟图里面不一样?大端小端)

![image-20191126140853639](img/NTLM_RELAY_2_ANQUANKE/image-20191126140853639.png)

NTLMSSP_NEGOTIATE_EXTENDED_SESSIONSECURITY对应的是第13位，改为0，也就是0xe2818215

![image-20191126142621775](img/NTLM_RELAY_2_ANQUANKE/image-20191126142621775.png)

改下就行

![image-20191126135508861](img/NTLM_RELAY_2_ANQUANKE/image-20191126135508861.png)



http的话在packets.py里面的NTLM_Challenge类里面

![image-20191201162908250](img/NTLM_RELAY_2_ANQUANKE/image-20191201162908250.png)

Responder的NTLM SSP没有通用性，这个挺难受的，其他协议的话，大家自己找吧。跟下代码挺快的。

### 2. Net-NTLM v2的破解

​	Net-NTLM v2 现在也没有什么比较好用的破解方式，一般就是利用hashcat 离线爆破明文密码，能不能跑出来就看字典里面有没有了。![image-20191201160707021](img/NTLM_RELAY_2_ANQUANKE/image-20191201160707021.png)使用hashcat进行字典破解
...
![image-20191201161335057](img/NTLM_RELAY_2_ANQUANKE/image-20191201161335057.png)
...
1. impacket 的底下的`smbrelayx.py`![image-20191126190640410](img/NTLM_RELAY_2_ANQUANKE/image-20191126190640410.png)

2. impacket 的底下的`ntlmrelayx.py`![image-20191201181600687](img/NTLM_RELAY_2_ANQUANKE/image-20191201181600687.png)

3. Responder底下的`MultiRelay.py`![image-20191129180841482](img/NTLM_RELAY_2_ANQUANKE/image-20191129180841482.png)![image-20191129180919113](img/NTLM_RELAY_2_ANQUANKE/image-20191129180919113.png)
...
​	下面演示通过[NtlmRelayToEWS](https://github.com/Arno0x/NtlmRelayToEWS.git)(事实上，工具挺多的。其他的大家可以上github自己找)来实现Relay2ews![image-20191128171830112](img/NTLM_RELAY_2_ANQUANKE/image-20191128171830112.png)![image-20191128171500527](img/NTLM_RELAY_2_ANQUANKE/image-20191128171500527.png)![image-20191128171731605](img/NTLM_RELAY_2_ANQUANKE/image-20191128171731605.png)
...
放置于web服务器。在[NtlmRelayToEWS](https://github.com/Arno0x/NtlmRelayToEWS.git) 里面通过-u 参数指定。![image-20191129190842538](img/NTLM_RELAY_2_ANQUANKE/image-20191129190842538.png)![image-20191129191508154](img/NTLM_RELAY_2_ANQUANKE/image-20191129191508154.png)
...
![image-20191129160918694](img/NTLM_RELAY_2_ANQUANKE/image-20191129160918694.png)
...
![image-20191129165423234](img/NTLM_RELAY_2_ANQUANKE/image-20191129165423234.png)

![image-20191129165352363](img/NTLM_RELAY_2_ANQUANKE/image-20191129165352363.png)
...
![image-20191129164053515](img/NTLM_RELAY_2_ANQUANKE/image-20191129164053515.png)

![image-20191129164621268](img/NTLM_RELAY_2_ANQUANKE/image-20191129164621268.png)
...
![image-20191129155114943](img/NTLM_RELAY_2_ANQUANKE/image-20191129155114943.png)

## 0x03 引用

- 1. [LM-Hash、NTLM-Hash、Net-NTLMv1、Net-NTLMv2详解](http://d1iv3.me/2018/12/08/LM-Hash、NTLM-Hash、Net-NTLMv1、Net-NTLMv2详解/)
  2. [The NTLM Authentication Protocol and Security Support Provider](http://davenport.sourceforge.net/ntlm.html)
